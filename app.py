...
def fetch_and_analyze(key):
    ...
    if len(data) < 4: return None # NEED at least 4 candles to calculate SMA(10) and trigger logic
    ...
    # Calculate Logic safely
    # iloc[-2] is the last closed candle. We need iloc[-3] for the previous one.
    last = df.iloc[-2]
    prev = df.iloc[-3]
...
for sym in symbols:
    ...
    if not result: continue # Gracefully skip if data is missing
    ...
    if signal:
        ...
        if st.session_state.last_alerts.get(sym) != t_id:
            ...
            # GitHub & Email (Wrapped in try/except so one failure doesn't crash app)
            try:
